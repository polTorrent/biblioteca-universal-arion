#!/bin/bash
# worker-watchdog.sh v3 — Comprova si el Venice Worker corre i el reinicia si no
# S'executa cada 5 minuts via cron
#
# Millores v3:
#   - Actualitzat per Venice AI (DeepSeek V4 Pro)
...
if [ -f "$LOCK_FILE" ]; then
    ...
    if [ -n"$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
        ...
        # Comprovar si hi ha tasques a running/ sense procés Venice fill
        RUNNING_COUNT=$(ls "$TASKS_DIR/running/"*.json 2>/dev/null | wc -l)
        if [ "$RUNNING_COUNT" -gt 0 ]; then
            # Hi ha tasca running — comprovar que hi ha un procés Venice actiu
            if ! pgrep -P "$OLD_PID" > /dev/null 2>&1 && ! pgrep -f "venice-worker" > /dev/null 2>&1; then
                # Tasca running però cap procés Venice — el worker pot estar encallat
                log "Tasca a running/ però cap procés Venice actiu. Possible encallament."
...
# ── 2. Comprovar si hi ha processos venice-worker actius sense lock ───────────
if pgrep -f "venice-worker" > /dev/null; then
    log "Procés Venice Worker actiu detectat sense lock. Sortint."
    exit 0
fi
...
log "Iniciant Venice Worker ($PENDING_COUNT tasques pending)..."
cd "$HOME/biblioteca-universal-arion"
source ~/.nvm/nvm.sh 2>/dev/null
nohup bash "$WORKER_SCRIPT" >> "$LOG" 2>&1 &
log "Venice Worker iniciat (PID $!)"