...
/**
 * ═══════════════════════════════════════════════════════════════════
 * ÍNDEX PLEGABLE I CONTROL DE NOTES
 * ═══════════════════════════════════════════════════════════════════
 */

document.addEventListener('DOMContentLoaded', function() {
    // Toggle índex visibility
    const toggleIndexBtn = document.getElementById('toggle-index');
    const workLayout = document.querySelector('.work-layout');
    const chapterNavAside = document.getElementById('chapter-nav-aside');

    if (toggleIndexBtn && workLayout) {
        // Crear botó flotant per mostrar l'índex
        const showIndexBtn = document.createElement('button');
        showIndexBtn.className = 'show-index-btn hidden';
        showIndexBtn.textContent = 'Índex';
        showIndexBtn.setAttribute('aria-label', 'Mostrar índex');
        document.body.appendChild(showIndexBtn);

        // Recuperar estat guardat
        const indexHidden = localStorage.getItem('indexHidden') === 'true';
        if (indexHidden) {
            workLayout.classList.add('index-hidden');
            showIndexBtn.classList.remove('hidden');
            toggleIndexBtn.textContent = 'Mostrar índex';
        }

        toggleIndexBtn.addEventListener('click', function() {
            const isHidden = workLayout.classList.toggle('index-hidden');
            showIndexBtn.classList.toggle('hidden', !isHidden);
            toggleIndexBtn.textContent = isHidden ? 'Mostrar índex' : 'Amagar índex';
            localStorage.setItem('indexHidden', isHidden);
        });

        showIndexBtn.addEventListener('click', function() {
            workLayout.classList.remove('index-hidden');
            showIndexBtn.classList.add('hidden');
            toggleIndexBtn.textContent = 'Amagar índex';
            localStorage.setItem('indexHidden', 'false');
        });
    }

    // Toggle collapse índex (només plega la llista)
    const collapseNavBtn = document.getElementById('collapse-nav');
    if (collapseNavBtn && chapterNavAside) {
        collapseNavBtn.addEventListener('click', function() {
            const isCollapsed = chapterNavAside.classList.toggle('collapsed');
            collapseNavBtn.textContent = isCollapsed ? '+' : '−';
            collapseNavBtn.setAttribute('aria-label', isCollapsed ? 'Desplegar índex' : 'Plegar índex');
        });
    }

    // Toggle notes visibility
    const mainContent = document.querySelector('.main-content');
    const viewBtns = document.querySelectorAll('.view-btn[data-mode]');
    ...
    viewBtns.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (mainContent) {
                if (mode === 'sense-notes') {
                    mainContent.classList.add('notes-hidden');
                } else {
                    mainContent.classList.remove('notes-hidden');
                }
            }

            // Guardar preferència
            localStorage.setItem('notesMode', mode);
        });
    });

    // Recuperar mode notes guardat
    const savedNotesMode = localStorage.getItem('notesMode');
    if (savedNotesMode && mainContent) {
        if (savedNotesMode === 'sense-notes') {
            mainContent.classList.add('notes-hidden');
        }
        viewBtns.forEach(btn => {
            btn.classList.toggle('active', btn.getAttribute('data-mode') === savedNotesMode);
        });
    }
});
