#!/usr/bin/env python3
"""Fetch all 20 Petőfi poems from Hungarian Wikisource API."""
import json
import re
import subprocess
import sys
...
def extract_hungarian_poem(wikitext: str, title: str) -> str:
    """Extract the Hungarian poem text from wikitext."""
    if not wikitext:
        return ""

    # Remove header template
    text = re.sub(r'\{\{fej\b.*?\}\}', '', wikitext, flags=re.DOTALL)
    # Remove other templates but keep content
    text = re.sub(r'\{\{[^}]*\}\}', '', text)
    # Remove categories
    ...
    if table_match:
        ...
        # Split by column separator: a line starting with just |
        parts = re.split(r'\n\|\s*\n', table_content)
        if len(parts) >= 2:
            text = parts[0]
        else:
            # Try splitting by | at start of line (single column marker)
            parts = re.split(r'\n\|\s*$', table_content, flags=re.MULTILINE)
            if len(parts) >= 2:
                text = parts[0]
            else:
                text = table_content
        # Remove table cell markup
...
# Exact Wikisource titles for each poem
POEMS = [
    ...
    ("Honfidal", "Honfidal"),
    ("Istentelen egy élet", "Istentelen egy élet..."),
]

# Alternative titles to try if the main one fails
ALT_TITLES = {
    "Honfidal": ["Honfidal (Petőfi Sándor)"],
    "Istentelen egy élet...": ["Istentelen egy élet…", "Istentelen egy élet"],
    "Az Alföld": ["Az alföld", "Az Alföld (Petőfi Sándor)"],
...
def main():
    ...
    for display_title, wiki_title in POEMS:
        print(f"\n--- {display_title} ---", file=sys.stderr)

        # Try main title
        wikitext = fetch_wikitext(wiki_title)
        if wikitext and len(wikitext) > 50:
            poem = extract_hungarian_poem(wikitext, display_title)
            if poem and len(poem) > 20:
                results[display_title] = poem
                print(f"  OK ({len(poem)} chars)", file=sys.stderr)
                time.sleep(0.5)
                continue

        # Try alternatives
        alts = ALT_TITLES.get(wiki_title, [])
        found = False
        for alt in alts:
            ...
            if wikitext and len(wikitext) > 50:
                ...
                if poem and len(poem) > 20:
                    results[display_title] = poem
                    print(f"  OK via alt '{alt}' ({len(poem)} chars)", file=sys.stderr)
                    found = True
                    break
            time.sleep(0.3)
