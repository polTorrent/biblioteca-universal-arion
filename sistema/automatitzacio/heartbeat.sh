        ...
        case "$action" in
            FETCH)
                if ! task_exists "$titol" > /dev/null 2>&1; then
                    add_task "fetch" "mkdir -p $obra_path && python3 sistema/traduccio/cercador_fonts_v2.py \"$autor\" \"$titol\" \"$lingua\" \"$obra_path\" && if [ ! -s $obra_path/original.md ]; then echo 'ERROR: No s.ha pogut obtenir original.md' && exit 1; fi && git add -A && git commit -m \"font: $titol de $autor\" && git push"
...
check_web_sync() {
    ...
    if [ "$obres_time" -gt "$docs_time" ] 2>/dev/null; then
        if ! task_exists "build.py\|regenera.*web\|actualitza.*web" > /dev/null 2>&1; then
            add_task "publish" "Web desactualitzada. Executa 'python3 sistema/web/build.py' per regenerar docs/. Verifica que les obres amb .validated apareixen. Commit i push."
...
if obra_path:
    ...
    if task_type in ('translate', 'fetch', 'translation'):
        if has_original:
            new_instruction = f'python3 sistema/traduccio/traduir_pipeline.py {obra_path}/'
            task_type = 'translate'
        else:
            ...
            new_instruction = f'mkdir -p {obra_path} && python3 sistema/traduccio/cercador_fonts_v2.py \\\"{autor}\\\" \\\"{titol}\\\" \\\"{llengua}\\\" \\\"{obra_path}\\\" && if [ ! -s {obra_path}/original.md ]; then echo ERROR && exit 1; fi && git add -A && git commit -m \\\"font: {titol}\\\" && git push'
            task_type = 'fetch'
    ...
    while IFS= read -r validated_file; do
        ...
        if [ -f "$PROJECT/docs/index.html" ] && ! grep -q "$obra_name" "$PROJECT/docs/index.html" 2>/dev/null; then
            if [ "$rebuilt_web" = false ]; then
                ...
                python3 "$PROJECT/sistema/web/build.py" 2>/dev/null && rebuilt_web=true
