...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def _ex(self):
        try:
            ...
            ok=any(c.startswith(p) for p in ['tail','head','cat','ls','grep','wc','find','ps aux','df','free','uptime','git log','git status','git diff','bash ~/biblioteca-universal-arion/sistema/automatitzacio/','kill ','pkill -f claude-worker','pkill -f dashboard','source ~/.nvm/nvm.sh','bankr'])
...
function workerCmd(action){const el=document.getElementById('worker-info');el.textContent='Executant...';let cmd='';if(action==='restart'){cmd='kill $(cat /tmp/worker.pid) 2>/dev/null; sleep 2; bash ~/biblioteca-universal-arion/sistema/automatitzacio/start-worker.sh'}else if(action==='stop'){cmd='kill $(cat /tmp/worker.pid) 2>/dev/null; rm -f ~/biblioteca-universal-arion/sistema/tasks/worker.lock'}else if(action==='status'){cmd='ps aux | grep venice-worker | grep -v grep; echo "---"; cat /tmp/worker.pid 2>/dev/null; echo "---"; ls ~/biblioteca-universal-arion/sistema/tasks/*.json 2>/dev/null | wc -l'}fetch('/api/execute',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command:cmd})}).then(r=>r.json()).then(d=>{if(d.error)el.textContent='Error: '+d.error;else el.textContent=(d.stdout||'').slice(-200);setTimeout(rf,2000)}).catch(e=>el.textContent='Error: '+e.message)}
